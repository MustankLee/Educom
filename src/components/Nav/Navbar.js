  ...
  return (
    <div className="absolute w-screen">
      <div className="flex justify-between pl-3 h-12 pr-8 items-center py-8 bg-slate-200 relative z-40">
        <NavLink to="/*"><img src={Educom} alt="icon" className="w-28" /></NavLink>
        <button onClick={clicked} className="border-blue-900 border-2 p-1 rounded-md text-blue-900 text-xl hover:bg-blue-900 hover:text-white tr">{show ? <IoClose/> : <RxHamburgerMenu/>}</button>
      </div>
      <nav className={show ? "flex h-screen w-1/5 absolute right-0 top-0 z-10" : "hidden"}>
